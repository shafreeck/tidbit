...
func (s *testCommitterSuite) TestPkNotFound(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/shafreeck/tidbit/tikv/txnNotFoundRetTTL", "return"), IsNil)
	err = txn3.LockKeys(context.Background(), lockCtx, k3)
	c.Assert(err.Error(), Equals, ErrLockAcquireFailAndNoWaitSet.Error())
	c.Assert(failpoint.Disable("github.com/shafreeck/tidbit/tikv/txnNotFoundRetTTL"), IsNil)
}
