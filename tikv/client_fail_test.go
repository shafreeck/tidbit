...
func (s *testClientFailSuite) TestPanicInRecvLoop(c *C) {
	c.Assert(failpoint.Enable("github.com/shafreeck/tidbit/tikv/panicInFailPendingRequests", `panic`), IsNil)
	c.Assert(failpoint.Enable("github.com/shafreeck/tidbit/tikv/gotErrorInRecvLoop", `return("0")`), IsNil)
	...
	c.Assert(failpoint.Disable("github.com/shafreeck/tidbit/tikv/gotErrorInRecvLoop"), IsNil)
	c.Assert(failpoint.Disable("github.com/shafreeck/tidbit/tikv/panicInFailPendingRequests"), IsNil)
	time.Sleep(time.Second)
