...
func (s *testLockSuite) TestZeroMinCommitTS(c *C) {
	...
	c.Assert(failpoint.Enable("github.com/shafreeck/tidbit/tikv/mockZeroCommitTS", mockValue), IsNil)
	s.prewriteTxnWithTTL(c, txn.(*tikvTxn), 1000)
	c.Assert(failpoint.Disable("github.com/shafreeck/tidbit/tikv/mockZeroCommitTS"), IsNil)
